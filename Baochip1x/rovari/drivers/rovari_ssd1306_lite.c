...
/* Minimal vsnprintf: %d %u %x %s %c %%. No heap, no newlib. */
static int mini_vsnprintf(char *buf, int size, const char *fmt, va_list ap)
{
    int pos = 0;
    while (*fmt && pos < size - 1) {
        ...
        if (*fmt == 'd') {
            ...
            while (t > 0 && pos < size - 1) buf[pos++] = tmp[--t];
            fmt++;
        } else if (*fmt == 'u') {
            ...
            if (s) while (*s && pos < size - 1) buf[pos++] = *s++;
            fmt++;
        } else if (*fmt == 'c') {
...
{
    SEVS_INVARIANT(oled_pages <= SSD1306_MAX_PAGES);
    memset(&tx_buf[1], 0, SSD1306_WIDTH);
    tx_buf[0] = 0x40;
    for (uint8_t p = 0; p < oled_pages; p++) {
        oled_set_pos(p, 0);
        i2c_write_raw(oled_i2c, oled_addr, tx_buf, SSD1306_WIDTH + 1);
    }
}
