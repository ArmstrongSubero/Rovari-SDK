...
#include <cstdarg>
/* Minimal vsnprintf for C++ (no newlib dependency) */
static inline int _oled_mini_vsnprintf(char *buf, int size, const char *fmt, va_list ap)
{
    int pos = 0;
...
public:
    ...
    void printLine(uint8_t line, const char* fmt, ...) __attribute__((format(printf, 3, 4))) {
        ...
        va_list ap;
        va_start(ap, fmt);
        _oled_mini_vsnprintf(buf, sizeof(buf), fmt, ap);
        va_end(ap);
