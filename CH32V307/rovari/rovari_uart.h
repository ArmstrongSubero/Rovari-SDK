...
public:
     ...
     */
    Uart() : _inst(SERIAL1), _initialized(false) {}
     ...
     *   serial2.begin(9600);
     */
    Uart(UartInstance inst) : _inst(inst), _initialized(false) {}

    /**
     * Construct and initialize immediately.
     *   Uart serial(SERIAL1, 115200);
     */
    Uart(UartInstance inst, uint32_t baud) : _inst(inst), _initialized(true) {
        uart_init(inst, baud);
    }
