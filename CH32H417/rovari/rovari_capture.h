 ...
 *
 * rovari_capture.h - Timer input capture (pulse measurement)
 *
 * Measures:
 *   - Pulse width (high time or low time)
 *   - Signal period (time between consecutive edges)
 *   - Frequency (derived from period)
 *
 ...
 *
 * Usage:
 *   capture_init(PD12, CaptureRising);
 *   uint32_t period_us = capture_read_period_us(PD12);
 *   uint32_t freq      = capture_read_freq(PD12);
 */
 ...
 *
 *   capture_init(PD12, CaptureRising);   // Capture rising edges on PA0
 *
